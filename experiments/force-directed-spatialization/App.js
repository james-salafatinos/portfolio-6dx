...
export default class Experiment {
  ...
  async start() {
    ...
    const settings = {
      repulsionStrength: this.game.repulsionStrength,
      attractionStrength: this.game.attractionStrength,
      wallRepulsionStrength: this.game.wallRepulsionStrength,
    };
    this.gui.add(settings, 'repulsionStrength', -1, 1, 0.01).name('Repulsion').onChange((v) => this.game.setRepulsionStrength(v));
    this.gui.add(settings, 'attractionStrength', 0, 1, 0.01).name('Attraction').onChange((v) => this.game.setAttractionStrength(v));
    this.gui.add(settings, 'wallRepulsionStrength', 0, 1, 0.01).name('Wall Repulsion').onChange((v) => this.game.setWallRepulsionStrength(v));
